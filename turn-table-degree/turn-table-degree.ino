...
float minuten = 1;           // Einstellwert für die Mintuen unlimited //(1-180min erlaubt)
unsigned long degree = 0;           // Einstellwert für die Mintuen (0-360min erlaubt), 0 means no limit
int stepperDirection;  // Drehrichtung mit dem uhrzeigersinn
int stepperRawDirection = 0;  // Drehrichtung gegen den uhrzeigersinn
int stepperInf = 0;        // Dreh dich nicht unendlich lange
int is_minute = 0;        // 0: time in minutes, 1: time in seconds, 2: time in hours, 3: degree
int back_forth = 0;      // Change direction after the angle is reached, ignored in infinite mode
...
const int stepsPerRevolutionRaw = 2050;  // change this to fit the number of steps per revolution
unsigned int stepsPerRevolution;
unsigned int stepsChangeDirection;
unsigned int formelWert;
...
Stepper myStepper(stepsPerRevolutionRaw, motorPin1, motorPin3, motorPin2, motorPin4);
...
void setup() {
  ...
  lcd.print(" TURN-TABLE v3.0"); // Ausgabe des Textes "Nachricht".
}
...
void loop() {
  ...
  if (progStatus == 1) {
      ...
      case Tasterselect:
        {
          ...
          {
            if (degree == 0) {
              stepsPerRevolution = stepsPerRevolutionRaw;
            } else {
              stepsPerRevolution = int(stepsPerRevolutionRaw * degree / 360);
            }
            formelWert = 60000 / stepsPerRevolution;
            
            if (is_minute % 4 == 0) {
              period = minuten * formelWert;   // aus eingestelltem Minutenwert period errechnen
            } else if (is_minute % 4 == 1) { // seconds
              period = minuten * formelWert / 60;
            } else if (is_minute % 4 == 2) { // hours
              period = minuten * formelWert * 6;
            } else if (is_minute % 4 == 3) { // degree do nothing
              break;
            }

            stepperInf = stepperInf % 2;
            back_forth = back_forth % 2;
            stepsChangeDirection = stepsPerRevolution;
            if (back_forth == 1) {
              period = period / 2;
            ...
            Serial.print ("stepperInf:  ");      // Testausgabe
            Serial.println (stepperInf);
            Serial.print ("back_forth:  ");      // Testausgabe
            Serial.println (back_forth);
            Serial.print ("stepsPerRevolution:  ");      // Testausgabe
    ...
    if (stepperRawDirection % 4 >= 2 and stepperInf % 2 == 0) {
      ...
      stepperDirection = 1;
      lcdstr2 = (String)char(1) + (String)char(3);
    } else if (stepperRawDirection % 4 >= 2 and stepperInf % 2 == 1) {
      // im Uhrzeigersinn infinite
      stepperDirection = 1;
      lcdstr2 = (String)char(1) + (String)char(6);
    } else if (stepperRawDirection % 4 >= 0 and stepperInf % 2 == 0) {
      // im Uhrzeigersinn
      stepperDirection = -1;
      lcdstr2 = (String)char(5) + (String)char(3);
    } else if (stepperRawDirection % 4 >= 0 and stepperInf % 2 == 1) {
      // im Uhrzeigersinn infinite
      stepperDirection = -1;
      lcdstr2 = (String)char(5) + (String)char(6);
    }

    lcdstr2 = lcdstr2 + (String)(back_forth % 2);

    if (is_minute % 4 == 0) {
      //Serial.print ("min:  ");
      lcdstr2 = formatStrR((String)int(minuten), 3) + " m/U " + formatStrR((String)int(degree), 3) + "  " + lcdstr2;
    } else if (is_minute % 4 == 1) {
      //Serial.print ("sec:  ");
      lcdstr2 = formatStrR((String)int(minuten), 3) + " s/U " + formatStrR((String)int(degree), 3) + "  " + lcdstr2;
    } else if (is_minute % 4 == 2) {
      //Serial.print ("hou:  ");
      lcdstr2 = formatStrR((String)int(minuten), 3) + " h/U " + formatStrR((String)int(degree), 3) + "  " + lcdstr2;
    } else if (is_minute % 4 == 3) {
      //Serial.print ("deg:  ");
      lcdstr2 = formatStrR((String)int(degree), 3)  + " deg - nogo " + (String)(back_forth % 2);
    }
    //Serial.println (lcdstr2);
    lcd.print(lcdstr2);

    if (progStatus == 2) {
      lcd.setCursor(12, 1);            // Anzeige eines ! = Rotation läuft
      lcd.print("!"); 
      startMillis = millis();  //initial start time
    }
    ...
    if (!stepperStop) {
      ...
      if (currentMillis - startMillis >= period)  //test whether the period has elapsed
      {
        ...
        if ((stepCount % stepsChangeDirection == 0) && (degree != 0)) {
         stepperDirection = stepperDirection * -1;
        } 

        if ((stepCount == stepsPerRevolution) && (stepperInf == 0)) {
