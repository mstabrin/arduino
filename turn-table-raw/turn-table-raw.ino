...
int minuten = 1;           // Einstellwert für die Mintuen (1-180min erlaubt)
int stepperDirection = 1;  // Drehrichtung gegen den uhrzeigersinn
...
unsigned long period = 29;   //milliseconds / every step
...
void setup() {
  ...
  lcd.print("-- TURN-TABLE --"); // Ausgabe des Textes "Nachricht".
}
...
void loop() {
  ...
  if (progStatus == 1) {
    ...
    lcdstr2 = formatStrR((String)minuten, 3) + " min/U    " + lcdstr2;
    lcd.print(lcdstr2);
      ...
      case Tasterlinks: // Wenn die rechte Taste gedrückt wurde... Werte auf Grundwert setzen
        {
          if (progStatus == 1)
          {
            ...
            stepperDirection = 1;  // Drehrichtung gegen den uhrzeigersinn
